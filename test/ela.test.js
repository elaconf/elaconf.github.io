...
data.sponsors.metadata.forEach(function(post) {
  ...
  test(post.name, function(t) {
    ...
    if (post.level !== 'Opportunity Grant Donor') {
      t.notEqual(sponsorImg.indexOf(post.name.toLowerCase().replace(/ /g, '-').replace('.', '-') + '.svg'), -1, 'sponsor must have image: _includes/sponsors/' + post.name.toLowerCase().replace(/ /g, '-').replace('.', '-') + '.svg')
